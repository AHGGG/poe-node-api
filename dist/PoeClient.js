...
export class PoeClient {
    constructor(opts) {
        ...
        const { cookie, debug = false, fetch = globalFetch } = opts;
        this._fetch = fetch;
        this.headers = {
    ...
    setBotId() {
        ...
        for (let nickName of nickNames) {
            let displayName = DisplayName[nickName];
            ...
            if (envKeys.includes(botChatIdKey) && envKeys.includes(botIdKey)) {
                ...
                if (this.debug)
                    console.log(`read ${displayName}'s chatId(${this.bots[nickName].chatId}) and id(${this.bots[nickName].id})`);
            }
        }
    }
    /**
    ...
    async addChatBreak(botNickName) {
        ...
        if (!data.data) {
            if (this.debug)
                console.log("Can not clear context! data:", data);
        }
        return data;
    }
    /**
    ...
    async deleteMessage(messageIds) {
        ...
        if (!data.data) {
            if (this.debug)
                console.log("Can not delete message! data:", data);
        }
        return data;
    }
    /**
    ...
    async purgeAllMessage() {
        ...
        if (!data.data) {
            if (this.debug)
                console.log("Can not purgeAllMessage res:", data);
        }
        return data;
    }
    /**
    ...
    async getHistory(botNickName, count = 25, cursor = undefined) {
        ...
        while (count > 0) {
            ...
            if (msgs === undefined || msgs.pageInfo === undefined || msgs.edges === undefined || msgs.edges.length === 0) {
                if (this.debug)
                    console.log(`msgs === undefined || msgs.pageInfo === undefined || msgs.edges === undefined || msgs.edges.length === 0`);
                break;
            }
            ...
            if (this.debug)
                console.log(`count: ${count}, cursor: ${cursor}`);
        }
        return messages;
    }
    /**
    ...
    async getBots() {
        ...
        for (let i = 0; i < bot_list.length; i++) {
            ...
            let chatData = await this.getBotByNickName(NickName[displayName]);
            if (this.debug)
                console.log(`${displayName} | chatData:`, JSON.stringify(chatData, null, 2));
     ...
     */
    async getBotByNickName(botNickName, retry = 10, retryIntervalMs = 2000) {
        if (this.debug)
            console.log("\n=========================== getBotByNickName ===========================");
        ...
        for (let i = 0; i < retry; ++i) {
            if (i > 0) {
                if (this.debug)
                    console.log(`retry getBotByNickName, nickname: ${botNickName}, retry: ${i}/${retry}`);
    ...
    async makeRequest(request) {
        ...
        if (this.debug)
            console.debug("[before makeRequest] headers: ", headers, ", payload:", payload);
        const response = await this._fetch('https://poe.com/api/gql_POST', {
            method: 'POST',
            headers: headers,
            body: payload
    ...
    async extractFormkey(html) {
        ...
        if (this.debug)
            console.debug(`regExpExecArray:`, regExpExecArray, ` scriptText:`, scriptText);
    ...
    async updateAllBotInfo() {
        ...
        for (const botNickName in this.bots) {
            if (!this.bots[botNickName] || !this.bots[botNickName].chatId || !this.bots[botNickName].id) {
                if (this.debug)
                    console.log(`${botNickName} in this.bots is not valid!, this.bots[${botNickName}]:\n`, JSON.stringify(this.bots[botNickName], null, 2));
                continue;
            }
            const displayName = DisplayName[botNickName];
